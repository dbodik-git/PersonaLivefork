...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def decode_latents_svd(
        ...
        latents = rearrange(latents, "b c f h w -> (b f) c h w").to(self.vae.dtype)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        exp_cond_tensor_list = []
        mot_bbox_param = []
        for tgt_image, exp_image in zip(tgt_images, exp_images):
            exp_cond_tensor = self.cond_image_processor.preprocess(
                exp_image, height=224, width=224
            ).transpose(0, 1) # (c, 1, h, w)
            exp_cond_tensor_list.append(exp_cond_tensor)

            tgt_cond_tensor = self.cond_image_processor.preprocess(
                tgt_image, height=256, width=256
            ).to(device=device, dtype=self.pose_encoder.dtype)  # (1, c, h, w)
            tgt_cond_tensor = tgt_cond_tensor / 2 + 0.5 # to [0, 1]
            ref_motion_emb, motion_emb = self.pose_encoder.inference(ref_cond_tensor, tgt_cond_tensor)  # [1, c]
            mot_bbox_param.append(motion_emb)
        mot_bbox_param = torch.cat(mot_bbox_param, dim=0)  # (t, c)
        keypoints = draw_keypoints(mot_bbox_param).unsqueeze(2)
        keypoints = rearrange(keypoints, 'f c b h w -> b c f h w')
        keypoints = keypoints.to(device=device, dtype=self.pose_guider.dtype)
        pose_feas = self.pose_guider(keypoints)  # (t, c)

        exp_cond_tensor = torch.cat(exp_cond_tensor_list, dim=1)  # (c, t, h, w)

        exp_cond_tensor = exp_cond_tensor.unsqueeze(0) # (1, c, t, h, w)
        exp_cond_tensor = exp_cond_tensor.to(
            device=device, dtype=self.motion_encoder.dtype
        )
        context_scheduler = get_context_scheduler(context_schedule)

        if do_classifier_free_guidance:
            ref_exp_cond_tensor = self.cond_image_processor.preprocess(
                ref_exp_image, height=224, width=224
            ).to(device=device, dtype=self.motion_encoder.dtype)
            neg_motion_hidden_states = self.motion_encoder(ref_exp_cond_tensor)    # [b, c]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    pose_cond_input = torch.cat([exp_cond_tensor[:, :, c] for c in context])
                    motion_hidden_states = self.motion_encoder(pose_cond_input)    # [b, c]

                    pose_fea = torch.cat([pose_feas[:,:,c] for c in context], dim=0)
                ...
                latents, pred_original_sample = self.scheduler.step(
        ...
        if isinstance(self.vae, AutoencoderKL):
            images = self.decode_latents(pred_original_sample)  # (b, c, f, h, w)
        elif isinstance(self.vae, AutoencoderKLTemporalDecoder):
            images = self.decode_latents_svd(pred_original_sample)
